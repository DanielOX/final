...
<style media="screen">
	...
	body
  {
    ...
    background-image: url('robson-hatsukami-morgan-280156-unsplash.jpg');
  ...
  <div class="row card z-depth-3" style="margin:0 auto;width:50%">
		<div class="col card-content s12 m12 l12 ">
		 <form method="post" action="data.php" enctype="multipart/form-data">
          ...
          <input type="hidden" name="type_post" value="business">
          <br/>
				</div>
...
<br>
					<?php
											$conn = mysqli_connect("127.0.0.1", "root","","community");
											$sql3 = "SELECT * from user,business where user.user_id in (select user_id from `business`) ORDER BY business.post_id DESC";
                       ...
                       $review_query = "SELECT count(*) as total FROM post_review WHERE post_id = $post_review_id AND review=1 AND type ='business'";
                       $review_result = mysqli_query($conn,$review_query);
                       $review_row = mysqli_fetch_assoc($review_result);
                       $d_review_query = "SELECT count(*) as total FROM post_review WHERE post_id = $post_review_id AND review=0 AND type ='business'";
                      ...
                      <ul class="dropdown-content" id="dropdown'.$i.'">
                      												<li>
                      													<a href="delete_post.php?id='.$row['post_id'].'&type=business">Delete</a>
                      												</li>
                                              <li>
                      													<a href="edit_post.php?id='.$row['post_id'].'&type=business">Edit</a>
                      												</li>
...
$('.liker').on('click',function(){
  ...
  http.open('GET','post_review.php?pid='+post_id_like+'&type=1&type_cat=business',true);
  http.send();
...
$('.disliker').on('click',function(){
  ...
  http.open('GET','post_review.php?pid='+post_id_like+'&type=0&type_cat=business',true);
  http.send();
