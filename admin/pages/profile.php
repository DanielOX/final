...
if($_FILES['image']['name'] !== "")
{
  ...
  $sql = "UPDATE user SET email='$email',password='$pass',user_name = '$u_name',image='$image'  WHERE user_id=$uid";
...
{
  $sql = "UPDATE user SET email='$email',password='$pass',user_name = '$u_name' WHERE user_id=$uid";
  $result = mysqli_query($conn,$sql);
  if($result){
  $sql = "UPDATE user_information SET gender = '$gender',address='$address',studied_at='$institute',isUndergraduate='$graduation',works_at='$works_at' WHERE user_id=$uid";
  $result = mysqli_query($conn,$sql);
...
if($result){ header('location: admin_profile.php');}else{ echo "Couldn\t Update Fatal Error!"; }
